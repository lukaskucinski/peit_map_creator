...
// Generate SVG with specific colors
...
async function main() {
  ...
  // 32x32 icons - no background, just the icon
  // For dark browser themes: white icon on transparent
  const darkIcon32 = generateSvg(32, 'transparent', 'white');
  await renderPng(darkIcon32, path.join(publicDir, 'icon-dark-32x32.png'));

  // For light browser themes: black icon on transparent
  const lightIcon32 = generateSvg(32, 'transparent', 'black');
  await renderPng(lightIcon32, path.join(publicDir, 'icon-light-32x32.png'));
